...
public class Math3d
{
    ...
    public static bool LinePlaneIntersection(out Vector3 intersection, Vector3 linePoint, Vector3 lineVec, Vector3 planeNormal, Vector3 planePoint)
    {
        try
        {
            intersection = LinePlaneIntersection(linePoint, lineVec, planePoint, planeNormal);
            return true;
        }
        catch
        {
    ...
    public static Vector3 LinePlaneIntersection(Vector3 l0, Vector3 ld, Vector3 p0, Vector3 pn)
    {
        return l0 + Vector3.Dot(p0 - l0, pn) / Vector3.Dot(ld, pn) * ld;
    }
    ...
    /// <summary>
    /// Sorts points clockwise around a center point.
    /// It's assumed that all points are planar, and no points are overlapping
    /// </summary>
    /// <param name="point"></param>
    /// <param name="normal"></param>
    /// <param name="points"></param>
    /// <returns></returns>
    public static Vector3[] CircleSort(Vector3 center, Vector3 normal, Vector3[] points)
    {
        normal = normal.normalized;
        Vector3 key = (points[0] - center).normalized;
        SortedList<float, Vector3> clockwise = new SortedList<float, Vector3>();
        SortedList<float, Vector3> counter = new SortedList<float, Vector3>();
        clockwise.Add(Mathf.NegativeInfinity, points[0]);
        for(int i = 1; i < points.Length; i++)
        {
            Vector3 v = (points[i] - center).normalized;
            float sin = Vector3.Dot(normal, Vector3.Cross(key, v));
            float cos = Vector3.Dot(key, v);
            if(sin == 0)
            {
                clockwise.Add(Mathf.Infinity, points[i]);
            }
            else if(sin < 0)
            {
                clockwise.Add(cos / sin, points[i]);
            }
            else
            {
                counter.Add(cos / sin, points[i]);
            }
        }
        ...
        {
            output[i] = clockwise.Values[i];
        }
        for(int i = 0; i < counter.Count; i++)
        {
            output[clockwise.Count + i] = counter.Values[i];
        }
        return output;
    }

    public static bool Compare(Vector3 a, Vector3 b, float diff = 0.001f)
    {
        return (a - b).sqrMagnitude <= diff * diff;
    }

    public static bool AreLineSegmentsIntersecting(Vector3 aa, Vector3 ab, Vector3 ba, Vector3 bb, float percision = 0.001f)
    {
        //check equality case
        if(Compare(aa, ba) && Compare(ab, bb))
        {
            return true;
        }
        if(Compare(aa, bb) && Compare(ab, ba))
        {
            return true;
        }
        ...
        float dot = Mathf.Abs(Vector3.Dot(aVec.normalized, bVec.normalized));
        if(dot < 0.99f)
        {
            return false;
        }

        Vector3 oPointA = LinePlaneIntersection(aa, aVec, Vector3.zero, aVec);
        Vector3 oPointB = LinePlaneIntersection(ba, bVec, Vector3.zero, bVec);
        if(!Compare(oPointA, oPointB))
        {
            return false;
        }

        float min = -percision;
        float max = aVec.magnitude + percision;
        aVec = aVec.normalized;

        bool test1 = false;
        {
            bool sub1 = false, sub2 = false;
            float BAonA = Vector3.Dot(aVec, ba - aa);
            if(BAonA > min && BAonA < max)
            {
                test1 = true;
                sub1 = true;
            }

            float BBonA = Vector3.Dot(aVec, bb - aa);
            if(BBonA > min && BBonA < max)
            {
                test1 = true;
                sub2 = true;
            }
        ...
        max = bVec.magnitude + percision;
        bVec = bVec.normalized;

        bool test2 = false;
        {
            bool sub1 = false, sub2 = false;
            float AAonB = Vector3.Dot(bVec, aa - ba);
            if(AAonB > min && AAonB < max)
            {
                test2 = true;
                sub1 = true;
            }

            float ABonB = Vector3.Dot(bVec, ab - ba);
            if(ABonB > min && ABonB < max)
            {
                test2 = true;
                sub2 = true;
            }
