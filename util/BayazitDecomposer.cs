...
public static class BayazitDecomposer
{
    ...
    public static List<List<Vector3>> ConvexPartition(List<Vector3> verticies, Func<Vector3, Vector2> v3ToV2 = null, Func<Vector2, Vector3> v2ToV3 = null)
    {
        if(v3ToV2 == null || v2ToV3 == null)
        {
            Vector3 a = verticies[1] - verticies[0];
            Vector3 b = verticies[2] - verticies[0];
            ...
            {
                int bIndex = 3;
                while(Mathf.Abs(Vector3.Dot(a, b)) > 0.9999f)
                {
                    b = (verticies[bIndex++] - verticies[0]).normalized;
                }
            }
            catch
            {
                throw new Exception("Polygon is all along a single line.");
            }

            Vector3 normal = Vector3.Cross(a.normalized, b.normalized);
            Quaternion rot = Quaternion.LookRotation(normal);
            Quaternion inverse = Quaternion.Inverse(rot);
            Vector3 offset = verticies[0];
            v3ToV2 = (x) => inverse * (x - offset);
            v2ToV3 = (x) => (rot * x) + offset;
        }

        List<Vector2> inputList = verticies.Select(x => v3ToV2(x)).ToList();
        var output = ConvexPartition(inputList);
        return output.Select(x => x.Select(y => v2ToV3(y)).ToList()).ToList();
    }
